...
void binary(int x, int *arr)
{
    ...
    // Printing logic often found at end of binary conversion in this text
...
int main()
{
    ...
    int s = 0, z = 0, Q[4], M[4], temp[4], temp1[4], ans[8], y = 0, x = 0, c = 0;
    ...
    // if (((a < 0) && (b > 0)) || ((a > 0) && (b < 0)))
    // {
    //     // Handling negative result sign if necessary based on inputs
    //     // Note: Booth's naturally handles signed numbers, 
    //     // but this specific code adds an extra conversion step.
    //     for (i = 0; i < 8; i++) ans[i] = 1 - ans[i];
    //     c = 1;
    //     for (i = 7; i >= 0; i--)
    //     {
    //         x = ans[i];
